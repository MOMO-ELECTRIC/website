...
const MATCH_FILE = process.env.LARK_MATCH_OUTPUT || path.resolve(process.cwd(), 'output', 'lark_paid_matches.json');
...
async function main() {
  ...
  const crhField = (fieldsResp.data?.items || []).find(f => (f.field_name || '').trim() === 'CRH_NO');
  if (!crhField) throw new Error('CRH_NO field not found');

  const matches = JSON.parse(fs.readFileSync(MATCH_FILE, 'utf8')).matches || [];
  const updates = matches
    .filter(m => m.matchStatus === 'high' && m.bestMatch?.recordId && m.applicationId)
    .map(m => ({ recordId: m.bestMatch.recordId, applicationId: m.applicationId, address: m.evhomeAddress }));

  const results = [];
  for (const u of updates) {
    if (!DRY_RUN) {
      await api(`https://open.larksuite.com/open-apis/bitable/v1/apps/${APP_TOKEN}/tables/${TABLE_ID}/records/${u.recordId}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ fields: { [crhField.field_name]: u.applicationId } })
      });
    }
    results.push(u);
  }

  console.log(JSON.stringify({ dryRun: DRY_RUN, field: crhField.field_name, count: results.length, updates: results }, null, 2));
}
